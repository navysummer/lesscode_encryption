...
impl SMX {
    ...
    pub fn sm4_encrypt(mode: String, data: String, key: String, iv: Option<String>) -> Result<String, SMXError> {
        if mode == "ecb" {
            Ok(sm4_ecb_encrypt_hex(&data, &key))
        } else if mode == "cbc" {
            match iv {
                Some(iv) => Ok(sm4_cbc_encrypt_hex(&data, &key, &iv)),
                None => Err(SMXError::MissingParameters)
            }
        } else {
    ...
    pub fn sm4_decrypt(mode: String, data: String, key: String, iv: Option<String>) -> Result<String, SMXError> {
        if mode == "ecb" {
            Ok(sm4_ecb_decrypt_hex(&data, &key))
        } else if mode == "cbc" {
            match iv {
                Some(iv) => Ok(sm4_cbc_decrypt_hex(&data, &key, &iv)),
                None => Err(SMXError::MissingParameters)
            }
        } else {
            Err(SMXError::InvalidMode)
